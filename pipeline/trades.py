...
from inference import load_pretrained_modoel
...
def main():
    ...
    train_dataset_config = DATASET_CONFIG["alignment"]
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            labels[labels == tokenizer.pad_token_id] = -100 # tokenizer.pad_token_id 50280
            ...
            if "openflamingo" not in args.instruction_type:
                for i in range(labels.shape[0]):
                    ...
                    while token_idx < labels.shape[1] and labels[i][token_idx] != answer_token_id:
                        ...
                        token_idx += 1
                    # answer_token_id 50279
            labels = labels.to(input_ids.device)
            labels[labels == answer_token_id] = -100
            labels[labels == media_token_id] = -100 # media_token_id 50278
            ...
            with torch.no_grad():
                clean_logits = model.module.forward_trades(
            ...
            with torch.cuda.amp.autocast(dtype=torch.float16):
                loss = model.module.forward_trades(
            ...
            # grad = torch.autograd.grad(loss, noise, create_graph=False, retain_graph=True)[0]
            # with torch.no_grad():
            #     noise = noise.detach() + at_config.at_alpha_imgs * grad.sign()
            #     noise = torch.clamp(noise, -at_config.at_eps_imgs, at_config.at_eps_imgs)
            # del clean_logits, perturbed_imgs,  grad
