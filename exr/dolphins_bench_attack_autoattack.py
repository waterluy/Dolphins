...
class AutoAttackLossWrapper(torch.nn.Module):
    def __init__(
        self,
        wrapper_model,
        wrapper_vision_x,
        ...
        super().__init__()
        self.wrapper_model = wrapper_model.half().cuda()
        self.wrapper_vision_x = wrapper_vision_x
        self.wrapper_input_ids = wrapper_input_ids
        self.wrapper_attention_mask = wrapper_attention_mask
        self.wrapper_labels = wrapper_labels

    def forward(self, x):
        with torch.cuda.amp.autocast():
            loss = self.wrapper_model.forward_auto_attack(
                vision_x=x.half().cuda(),
                lang_x=self.wrapper_input_ids.cuda(),
                attention_mask=self.wrapper_attention_mask.cuda(),
                labels=self.wrapper_labels.cuda(),
                media_locations=None,
                forward_type=ForwardType(FORWARDTYPE),
            )[0]
        # targets = self.labels[:, 1:]
        # logits = logits[:, :-1, :] if logits.shape[1] > targets.shape[1] else logits  # 对齐长度
        # loss = self.criterion(logits.reshape(-1, logits.shape[-1]), targets.flatten())
        return loss

def my_auto_attack(model, vision_x, input_ids, attention_mask, labels=None, epsilon=0.1, ):
    wrapped_model = AutoAttackLossWrapper(
        wrapper_model=model,
        wrapper_vision_x=vision_x.half().cuda(),
        wrapper_input_ids=input_ids,
        wrapper_attention_mask=attention_mask,
        wrapper_labels=labels,
    ).cuda()
    ...
    attacker.apgd.seed = attacker.get_seed()
    adv_imgs = attacker.apgd.perturb(vision_x.half().cuda())  

    noise = adv_imgs.cuda() - vision_x.cuda()
...
if __name__ == "__main__":
    ...
    with open(json_file, 'w') as file:
        ...
        for entry in tqdm(data):
            ...
            # pgd attack
            noise = my_auto_attack(model=model, vision_x=images.cuda(), input_ids=input_ids.cuda(), attention_mask=attention_mask, labels=labels, epsilon=args.eps)
