...
def pgd_attack(model, vision_x, input_ids, attention_mask, labels=None, epsilon=0.001, steps=10, lp='linf', dire='pos'):
    with torch.no_grad():
        clean_features = model.get_adapter_features(vision_x.half().cuda())
    ...
    for _ in range(steps):
        ...
        features = model.get_adapter_features(
            vision_x=vision_x_noise,
        )
        # lp 距离作为损失
        loss = torch.norm(features - clean_features, p=LP)  # L2 norm
        noise.grad = None
        loss.backward()
...
if __name__ == "__main__":
    ...
    if args.distance == 'l2':
        ...
        LP = 1
    elif args.distance == 'linf':
        LP = float('inf')
    else:
        raise ValueError(f"Unsupported distance metric: {args.distance}")
