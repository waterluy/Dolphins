...
for eps in "${eps_values[@]}"; do
    for iter in "${iter_values[@]}"; do
        for query in "${query_values[@]}"; do
            ...
            # 定义输出文件路径
            result=$((iter * query))
            coi_output_file="results/bench_attack_coi-wo-stage2-fix_eps${eps}_iter${result}/dolphin_output.json"
                    
            # 检查COI实验的输出文件是否存在
            if [ ! -f "$coi_output_file" ]; then
                echo "Running experiment COI with eps=$eps, iter=$iter query=$query result=$result"
                python attack/dolphins_bench_attack_wlu_xr_wo_stage2_fix.py --eps "$eps" --iter "$iter" --query "$query"    # >> output_eps_${eps}_iter_${iter}.log 2>&1
                python tools/dolphin_evaluate.py --exp bench_attack_coi-wo-stage2-fix_eps${eps}_iter${result}
