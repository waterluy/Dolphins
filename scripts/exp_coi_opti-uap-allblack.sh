...
for eps in "${eps_values[@]}"; do
    for iter in "${iter_values[@]}"; do
        for query in "${query_values[@]}"; do
            # 定义输出文件路径
            coi_output_file="results/bench_attack_coi-opti-uap-allblack_eps${eps}_iter${iter}_query${query}/dolphin_output.json"
            ...
            if [ ! -f "$coi_output_file" ]; then
                ...
                echo "Skipping COI-opti-uap-allblack experiment with eps=$eps, iter=$iter as output already exists"
            fi
