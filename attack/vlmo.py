...
def get_image_features(image):
    ...
    image_embeds= transformer(resize_to_224(image))
    # image_embeds = F.pad(image_embeds, (0, 1024 - 1000))
    return image_embeds.cuda()
...
def get_text_features(text):
    # text_token = model_clip.encode_text(clip.tokenize(text).cuda())
    text_input = new_tokenizer(text, padding='max_length', truncation=True, max_length=30, return_tensors="pt").to(device) 
    text_output = model_albef.text_encoder(text_input.input_ids, attention_mask = text_input.attention_mask, mode='text')  
    text_feat = text_output.last_hidden_state
    text_embed = model_albef.text_proj(text_feat[:,0,:])
    text_embed = F.normalize(text_embed,dim=-1)
    # inputs = new_tokenizer(text, truncation=True, padding=True, return_tensors='pt')
    # input_ids = inputs["input_ids"]
    # token_type_ids = inputs["token_type_ids"]
    # text_embed = text_embeddings(input_ids=input_ids, token_type_ids=token_type_ids)
    # text_embed = text_embeddings(text_output)
    text_embed = adapter(text_embed)
    return text_embed.cuda()
...
def text_supervision(
    ...
    image_features = get_image_features(normed_noisy_vision_x)
    if LOSS == 'cos':
        ...
        total_loss = - torch.cosine_similarity(image_features_normed, text_features_normed, eps=1e-8)
        # total_loss = [ - image_features @ text_feature.t() for text_feature in text_features]
        # print(total_loss) # torch.Size([16])
...
def inverse3p_supervision(
    ...
    normed_noisy_vision_x = normalize(noisy_vision_x, mean=image_mean, std=image_std)
    resize_to_224 = transforms.Resize((224, 224), interpolation=InterpolationMode.BICUBIC, max_size=None)
    # resize_to_384 = transforms.Resize((384, 384), interpolation=InterpolationMode.BICUBIC, max_size=None)
    # 定义目标文本和其他文本
    ...
    text_features = [get_text_features(text) for text in texts]
    img_feature = get_image_features(normed_noisy_vision_x)
    logits_per_image = [torch.cosine_similarity(img_feature, text_feature, eps=1e-8) for text_feature in text_features]
...
def clean_supervision(
    ...
    clean_features = get_image_features(ori_vision_x[0, 0, :].cuda())
    noise_features = get_image_features(normed_noisy_vision_x)
    if LOSS == 'cos':
...
def adj_supervision(
    ...
    text_features1 = get_text_features("A safe driving scenario.")
    text_features2 = get_text_features("A unsafe driving scenario.")
    adv_image = get_image_features(normed_noisy_vision_x)
    clean_image = get_image_features(ori_vision_x[0, 0, :].cuda())
    adv_logits_per_image = torch.stack((torch.cosine_similarity(adv_image, text_features1, dim=1, eps=1e-8).mean(),torch.cosine_similarity(adv_image, text_features2, dim=1, eps=1e-8).mean()),dim=0)
    adv_logits_per_image = torch.softmax(adv_logits_per_image, dim=-1).cuda()
    
    clean_logits_per_image = torch.stack((torch.cosine_similarity(clean_image, text_features1, dim=1, eps=1e-8).mean(),torch.cosine_similarity(clean_image, text_features2, dim=1, eps=1e-8).mean()),dim=0)
    clean_logits_per_image = torch.softmax(clean_logits_per_image, dim=-1).cuda()
    ...
    target_labels[ min_index] = 1
    mask = target_labels == 1
    # 最大化target label 同时抑制其他label
    bs = adv_logits_per_image.shape[0]
    loss = -torch.log(1e-8 + adv_logits_per_image[mask]).mean(dim=-1, keepdim=True) + torch.log(1e-8 + adv_logits_per_image[~mask]).mean(dim=-1, keepdim=True)
    loss = loss.mean(dim=0)
    return loss
...
if __name__ == "__main__":
    ...
    if args.sup_adj:
        ...
        iii += '-adj'
    folder = f'{args.output}/vlmo_eps0.1_iter30_query1'
    ...
    bert_config = BertConfig(
            vocab_size=30522,
            hidden_size=transformer.num_features,
    ...
    yaml = YAML(typ='rt')
    # config = yaml.load(open('ALBEF/configs/Retrieval_flickr.yaml', 'r'), Loader=yaml.Loader)
    config = yaml.load(open('ALBEF/configs/Retrieval_flickr.yaml', 'r'))
    model_name = 'google-bert/bert-base-uncased'
    new_tokenizer = BertTokenizer.from_pretrained(model_name)
    model_albef = ALBEF(config=config, text_encoder='bert-base-uncased', tokenizer=new_tokenizer)
    model_albef = model_albef.to(device)
    for param in model_albef.parameters():
        param.requires_grad = False
    adapter = AdapterLayer(256, 1000)
