...
def coi_attack_stage1(
    ...
    for q in range(QUERY):
        ...
        if ori_answer is None:
            ...
            if judge(ori=ori_answer, now=final_answer) < 25:
                break
    ...
    try:
        with open(json_path, 'a') as file:
            ...
            for entry in tqdm(data):
                ...
                induction_texts = list(filter(lambda x: x["unique_id"] == unique_id, induction_texts))
                noise, induction_answers = coi_attack_stage1(task=task_name, video_path=video_path, ori_vision_x=vision_x, instruction=instruction, ori_inputs=inputs)
