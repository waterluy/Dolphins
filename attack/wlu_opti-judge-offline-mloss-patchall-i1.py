...
def coi_attack_stage2(
    ...
    for _ in range(ITER):
        ...
        patch_start = noise_generator(text_features)
        if torch.isnan(patch_start).any():
            print(patch_start)
            quit()
        if args.sup_text:
...
if __name__ == "__main__":
    ...
    if args.sup_adj:
        iii += '-adj'
    folder = f'results/bench_attack_coi-opti-judge-offline-{LOSS}-patchall-i1{iii}_eps{EPS}_iter{ITER}_query{QUERY}'
