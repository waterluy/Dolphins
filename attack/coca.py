...
def text_supervision(
    ...
    if LOSS == 'cos':
        ...
        text_embeds, image_embeds = coca(text = text_features,images = resize_to_224(normed_noisy_vision_x),return_embeddings = True)
        total_loss = - torch.cosine_similarity(image_embeds, text_embeds, dim=1, eps=1e-8)
...
def inverse3p_supervision(
    ...
    logits_per_image = coca(text = text_tokens,images = resize_to_224(normed_noisy_vision_x))
...
def adj_supervision(
    ...
    text_embeds1, image_embeds = coca(text = text_features[0],images = resize_to_224(normed_noisy_vision_x),return_embeddings = True)
    text_embeds2, image_embeds = coca(text = text_features[1],images = resize_to_224(normed_noisy_vision_x),return_embeddings = True)
    adv_logits_per_image = torch.stack((torch.cosine_similarity( image_embeds,text_embeds1, dim=1, eps=1e-8).mean(),torch.cosine_similarity(image_embeds,text_embeds2, dim=1, eps=1e-8).mean()))
    ...
    text_embeds1, image_embeds = coca(text = text_features[0],images = resize_to_224(ori_vision_x[0, 0, :]).cuda(),return_embeddings = True)
    text_embeds2, image_embeds = coca(text = text_features[1],images = resize_to_224(ori_vision_x[0, 0, :]).cuda(),return_embeddings = True)
    clean_logits_per_image = torch.stack((torch.cosine_similarity( image_embeds,text_embeds1, dim=1, eps=1e-8).mean(),torch.cosine_similarity(image_embeds,text_embeds2, dim=1, eps=1e-8).mean()))
...
if __name__ == "__main__":
    ...
    if args.sup_adj:
        ...
        iii += '-adj'
    folder = f'{args.output}/coca_eps0.1_iter30_query1'
    ...
    coca = CoCa(
        dim = 512,                     # 模型维度
        img_encoder = vit,             # 视觉变压器 - 图像编码器，返回图像嵌入向量（批次，序列，维度）
        image_dim = 1024,              # 图像嵌入向量维度，如果与模型维度不同
        num_tokens = 20000,            # 文本令牌数量
        unimodal_depth = 6,            # 单模态变换器的深度
        multimodal_depth = 6,          # 多模态变换器的深度
        dim_head = 64,                 # 注意力头的维度
        heads = 8,                     # 注意力头的数量
        caption_loss_weight = 1.,      # 自回归标题损失的权重
        contrastive_loss_weight = 1.,  # 图像和文本CLS嵌入之间的对比损失的权重
    ).cuda()
