...
def get_image_features(image):
    ...
    image_feat = model_blip.visual_encoder(image)   
    image_embed = model_blip.vision_proj(image_feat[:,0,:])            
    image_embed = F.normalize(image_embed,dim=-1)
    return image_embed

def get_text_features(text):
    text_input = model_blip.tokenizer(text, padding='max_length', truncation=True, max_length=35, return_tensors="pt").to(device) 
    text_output = model_blip.text_encoder(text_input.input_ids, attention_mask = text_input.attention_mask, mode='text')  
    text_embed = F.normalize(model_blip.text_proj(text_output.last_hidden_state[:,0,:]))
    return text_embed
...
def adj_supervision(
    ...
    adv_logits_per_image = [get_sim( text,resize_to_384(normed_noisy_vision_x)) for text in texts]
    adv_logits_per_image = torch.stack(adv_logits_per_image)
    adv_logits_per_image = torch.softmax(adv_logits_per_image, dim=-1).cuda()
    clean_logits_per_image = [get_sim(text,resize_to_384(ori_vision_x[0, 0, :]).cuda()) for text in texts]
    clean_logits_per_image = torch.stack(clean_logits_per_image)
...
if __name__ == "__main__":
    ...
    if args.sup_adj:
        ...
        iii += '-adj'
    folder = f'{args.output}/blip_eps0.1_iter30_query1'
    ...
    config = yaml.load(open('BLIP/configs/retrieval_flickr.yaml', 'r'))
    model_blip = blip_retrieval(pretrained=config['pretrained'], image_size=config['image_size'], vit=config['vit'], 
