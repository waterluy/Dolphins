...
def get_image_features(image):
    ...
    image_feat = model_albef.visual_encoder(image)        
    image_embed = model_albef.vision_proj(image_feat[:,0,:])            
...
def get_sim(text,image):
    image_embeds = get_image_features(image)
    text_embeds = get_text_features(text)
    sims_matrix = image_embeds @ text_embeds.t()
    return sims_matrix.mean()
...
def text_supervision(
    ...
    image_features = get_image_features(resize_to_384(normed_noisy_vision_x))
    if LOSS == 'cos':
...
def inverse3p_supervision(
    ...
    logits_per_image = [get_sim(text,resize_to_384(normed_noisy_vision_x)) for text in texts]
...
def clean_supervision(
    ...
    clean_features = get_image_features(resize_to_384(ori_vision_x[0, 0, :]).cuda())
    noise_features = get_image_features(resize_to_384(normed_noisy_vision_x))
    if LOSS == 'cos':
...
def adj_supervision(
    ...
    adv_image = get_image_features(resize_to_384(normed_noisy_vision_x))
    clean_image = get_image_features(resize_to_384(ori_vision_x[0, 0, :].cuda()))
    adv_logits_per_image = torch.stack(((adv_image @ text_features1.t()).mean(),(adv_image @ text_features2.t()).mean()),dim=0)
    adv_logits_per_image = torch.softmax(adv_logits_per_image, dim=-1).cuda()
    
    clean_logits_per_image = torch.stack(((clean_image @ text_features1.t()).mean(),(clean_image @ text_features2.t()).mean()),dim=0)
...
if __name__ == "__main__":
    ...
    if args.sup_adj:
        ...
        iii += '-adj'
    folder = f'{args.output}/albef_eps0.1_iter30_query1'
