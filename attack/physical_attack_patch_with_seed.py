...
def coi_attack_stage1(
    ...
    # 初始化通用对抗补丁
    tmp = [Image.open(seed_path).convert('RGB') for _ in range(batch_size)]
    tmp = torch.stack([transform_totensor(image) for image in tmp], dim=0).to(device)
    resize = transforms.Resize(size=patch_size)
    adversarial_patch = resize(tmp)
    adversarial_patch.requires_grad = True
    alpha = 2 * EPS / ITER
