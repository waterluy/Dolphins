...
import defense.module_util as mutil


def get_nrp(purifier, device):
    if purifier == 'NRP':
        model = NRP(3,3,64,23)
        model.load_state_dict(torch.load('/home/beihang/wlu/defense/NRP/pretrained_purifiers/NRP.pth'))
    elif purifier == 'NRP_resG':
        model = NRP_resG(3, 3, 64, 23)
        model.load_state_dict(torch.load('/home/beihang/wlu/defense/NRP/pretrained_purifiers/NRP_resG.pth'))
    else:
