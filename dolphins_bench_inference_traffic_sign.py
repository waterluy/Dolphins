...
if __name__ == "__main__":
    ...
    folder = 'traffic pic'
    img_path_list = os.listdir(folder)
    ...
    with open(os.path.join('traffic_result', 'dolphin_output.json'), 'w') as file:
        for path in tqdm(img_path_list):
            frames = [Image.open(os.path.join(folder, path)).convert('RGB')]
            images = torch.stack([resize_to_336(transform_totensor(image)) for image in frames], dim=0).unsqueeze(0).unsqueeze(0).cuda()
            prompt = [
                f"USER: <image> is a traffic sign.Given the image,consider the standard meaning it conveys in traffic regulations and describe the action vehicles should take when encountering this traffic sign. GPT:<answer>"
            ]
            inputs = tokenizer(prompt, return_tensors="pt", ).to(model.device)
